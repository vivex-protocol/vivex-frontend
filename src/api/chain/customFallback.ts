import { Chain, HttpRequestError, createTransport, type CreateTransportErrorType, type Transport, type TransportConfig } from "viem"
...
export function customFallback(transports_: Transport[], config: FallbackTransportConfig = {}): FallbackTransport {
        ...
        async request({ method, params }) {
          const fetch = async (i = 0): Promise<any> => {
            ...
            try {
              ...
              const isEmptyRes = Number(response) === 0 || response === "0x"
              if (isEmptyRes && transport.config.name === "chainnodes" && chain?.id !== 100 && transports.length > 1) {
                console.log("[fallback] chainnodes returned an empty response")
                throw new Error("received empty response. Verify result by trying another provider")
              }
              ...
              // console.log(`
              //   [fallback]
              //   request: { method: ${method}, params: ${JSON.stringify(params, (_, v) => {
              //     if (typeof v !== "bigint") return v
              //     return v.toString() + "n"
              //   },
              //   2)} }
              //   failed on rpc: ${(transport.value as any).url}
              //   retrying on rpc: ${(transports[i + 1]({ chain, retryCount: 0, timeout }).value as any).url}
              // `)
